...
the string is then read back from the EEPROM and compared with the original one
  ...
  addr[0] = (uint8_t) ((0xA & 0xFF00) >> 8);
  addr[1] = (uint8_t) (0xA & 0xFF);
 ...
 // while(HAL_I2C_IsDeviceReady(&hi2c1, 0xA0, 1, HAL_MAX_DELAY) != HAL_OK);
  ...
  HAL_I2C_Master_Seq_Transmit_IT(&hi2c1, 0xA0, (uint8_t *)addr, 2, I2C_FIRST_AND_NEXT_FRAME) ;
  HAL_Delay(1) ;
  HAL_I2C_Master_Seq_Receive_IT(&hi2c1, 0xA0, (uint8_t *)rmsg, 7, I2C_LAST_FRAME) ;
