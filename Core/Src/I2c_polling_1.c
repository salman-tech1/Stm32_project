...
the string is then read back from the EEPROM and compared with the original one
   */
  Write_To_24LCxx(&hi2c1, 0xA0, 0xA, (uint8_t*)wmsg, strlen(wmsg)+1);


  HAL_Delay(500) ;

  Read_From_24LCxx(&hi2c1, 0xA0, 0xA, (uint8_t*)rmsg, strlen(wmsg)+1);

  HAL_UART_Transmit(&huart2, (uint8_t *)test, strlen(test), 0xfff);
